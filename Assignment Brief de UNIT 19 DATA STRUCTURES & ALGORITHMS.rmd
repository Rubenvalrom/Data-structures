...
6.  **Scribbr Citation Generator**:
    ...
    -   **Generación de Citas y Referencias**: Scribbr Citation
        Generator es una herramienta en línea que facilita la creación
        de citas y referencias, en este caso Harvard.
...
6.  **scipy.sparse.csr_matrix:** Representa una matriz dispersa en
    ...
    operaciones aritméticas y de almacenamiento.
...
2.  **Selección del Nodo Actual**:
    ...
    ![Dijkstra 1](imagenes/Step1Dij.png){width="384"}
...
3.  **Actualización de Distancias**:
    ...
    -   Compara la distancia recién calculada con la distancia actual
        asignada y asigna el menor valor.

    ![Dijkstra 2](imagenes/Step2Dij.png){width="391"}
...
4.  **Marcar Nodo como Visitado**:
    ...
    ![Dijkstra 3](imagenes/Step3Dij.png){width="390"}
...
for i in range(10000):
  ...
  indices = 0, return_predecessors=True, unweighted=False)
...
def crear_ruta(M, inicio, destinos):
  ...
  # Utilizar el algoritmo de Dijkstra para encontrar el nodo más cercano al inicio
  dist_matrix, predecessors = dijkstra(M, directed=True,
  unweighted= False, indices=inicio, return_predecessors=True)
  
  # Crear un diccionario con la distancia de cada nodo
  dist_dic = {index: value for index, value in enumerate(dist_matrix)}
  
  # Eliminar todos los valores que sean 0 y los nodos estén en destinos
  ...
  np.array(trazar_camino_intermedio(predecessors, inicio, nodo_más_cercano))))
  ...
  while len(destinos) > 0:
    # Utilizar el algoritmo de Dijkstra para 
    # encontrar la ruta más corta a cada nodo destino
    
    dist_matrix, predecessors = dijkstra(M, directed=True,
    indices=nodo_más_cercano, return_predecessors=True)
    
    nodo_anterior = nodo_más_cercano
    
    # Pasar la matriz a diccionario
    dist_dic = {index: value for index, value in enumerate(dist_matrix)}
...
#ox.save_graphml(G, filepath='mapa/madrid.graphml')
```

```{python}
# Importo el grafo guardado
madrid = ox.load_graphml('mapa/madrid.graphml')
...
print(f"Longitud total de la red de calles: {longitud} metros")
```

```{python}
# Mostrar el grafo de la red de calles de Madrid
ax = ox.plot_graph(madrid, bgcolor='w', node_color='b', 
                        node_size=1, edge_color='k',
                        edge_linewidth=0.5, figsize=(10, 10))
```
...
```{python}
np.random.seed(270621)

nodos_aleatorios = np.random.choice(matriz_madrid.shape[0], size=5, replace=False)

print("Nodos aleatorios seleccionados:")
...
print(f"La distancia total del recorrido es: {distancia} metros")
