...
```         
FUNCIÓN Dijkstra(Grafo G, Nodo s, Nodo t)
  
// Verificar si el grafo es conexo
  SI NoEsConexo(G) Entonces
    RETORNAR "Error: El grafo no es conexo"
  FIN SI

// Verificar si existe un camino desde el nodo origen s hasta el nodo destino t
  SI NoExisteCaminoDesdeS(HastaT, G) Entonces
    RETORNAR "No existe ruta entre los nodos s y t"
  FIN SI

// Inicializar la distancia desde el nodo origen s hasta cada nodo en el grafo
  Para cada nodo v en G hacer
    d[v] = INFINITO

// Inicializar la distancia desde el nodo origen s hasta el nodo s
  d[s] = 0

// Inicializar el predecesor de cada nodo en el camino más corto
  π[s] = NULL

  // Para cada nodo v en el grafo:
  Para cada nodo v en G hacer
    Si d[v] != INFINITO Entonces
      // Actualizar la distancia y predecesor del nodo v según sea necesario
      Para cada arista (u, v) en E hacer
        Si d[u] + peso(u, v) < d[v] Entonces
          d[v] = d[u] + peso(u, v)
          π[v] = u
        FIN SI
    FIN SI

// Verificar si la distancia desde el nodo s hasta el nodo t es infinita
  Si d[t] == INFINITO Entonces
    RETORNAR "La distancia entre los nodos s y t es infinita"
  FIN SI

// Construir el camino más corto desde el nodo origen s hasta el nodo destino t
  camino = []
  v = t
  Mientras π[v] != NULL Hacer
    camino.insertar al principio(π[v])
    v = π[v]
  FIN MIENTRAS

// Devolver el camino más corto y la distancia entre los nodos s y t
  RETORNAR (camino, d[t])

FUNCIÓN NoEsConexo(Grafo G)
// Verificar si el grafo es conexo
  SI ExisteCicloEnG(G) Entonces
    RETORNAR Verdadero
  FIN SI

  RETORNAR Falso

FUNCIÓN NoExisteCaminoDesdeS(HastaT, Grafo G)
// Verificar si existe un camino desde el nodo origen s hasta el nodo destino t
  Para cada arista (u, v) en E hacer
    Si u == s y v == t Entonces
      RETORNAR Verdadero
    FIN SI
  FIN PARA
...
FUNCIÓN peso(u, v)
// Devolver el peso de la arista entre los nodos u y v
  RETORNAR pesoDeLaArista(u, v)

FUNCIÓN ExisteCicloEnG(Grafo G)
// Verificar si existe un ciclo en el grafo
  Para cada nodo v en G hacer
    Si d[v] != INFINITO Entonces
    // Buscar un ciclo utilizando la función de búsqueda Depth First Search (DFS)
      SI HayCiclo(v, G) Entonces
        RETORNAR Verdadero
      FIN SI
    FIN SI
  FIN PARA
...
FUNCIÓN HayCiclo(nodo v, Grafo G)
// Buscar un ciclo utilizando la función de búsqueda DFS
  visited[v] = Verdadero
  Para cada arista (u, v) en E hacer
    Si u == v Entonces
      RETORNAR Verdadero
    FIN SI
    Si !visited[u] Entonces
      SI HayCiclo(u, G) Entonces
        RETORNAR Verdadero
      FIN SI
  FIN PARA
...
-   La función `NoEsConexo` verifica si el grafo es conexo.

-   La función `NoExisteCaminoDesdeS` verifica si existe un camino desde el nodo origen hasta el nodo destino.

-   La función `peso` devuelve el peso de la arista entre dos nodos.

-   Las funciones `ExisteCicloEnG`, `HayCiclo` y `visited` se utilizan para verificar si existe un ciclo en el grafo utilizando la función de búsqueda `DFS`.

    -   `DFS` :Su funcionamiento consiste en ir expandiendo todos y cada uno de los nodos que va localizando, de forma recurrente, en un camino concreto. Cuando ya no quedan más nodos que visitar en dicho camino, regresa de modo que repite el mismo proceso con cada uno de los hermanos del nodo ya procesado.
...
```         
FUNCIÓN TransformarGrafoAMontículoFibonacci(Grafo G)
// Verificar si la matriz de adyacencia es cuadrada
  SI NoEsCuadrada(G) Entonces
    RETORNAR "Error: La matriz de adyacencia no es cuadrada"
  FIN SI

// Obtener el tamaño del grafo (número de nodos)
  n = TamañoDeGrafo(G)
...
// Crear un montículo de Fibonacci vacío
  MontículoFibonacci M

// Recorrer los nodos del grafo y agregarlos al montículo de Fibonacci
  Para cada nodo i en G hacer
    Si GradoDeConexión(i) > 0 Entonces
      AgregarNodoAlMontículo(M, i, F[GradoDeConexión(i)])
    FIN SI

// Devolver el montículo de Fibonacci construido
  RETORNAR M

FUNCIÓN NoEsCuadrada(Grafo G)
// Verificar si la matriz de adyacencia es cuadrada
  SI TamañoDeGrafo(G) ≠ TamañoDeMatrizDeAdyacencia(G) Entonces
    RETORNAR Verdadero
  FIN SI

  RETORNAR Falso

FUNCIÓN AgregarNodoAlMontículo(MontículoFibonacci M, Nodo i, Altura h)
// Agregar el nodo i al montículo de Fibonacci con altura h
  InsertarNodoEnMontículo(M, i, h)

FUNCIÓN InsertarNodoEnMontículo(MontículoFibonacci M, Nodo i, Altura h)
// Insertar el nodo i en el lugar correcto del montículo de Fibonacci
  SI MontículoVacio(M) Entonces
    AgregarNodoAlRaíz(M, i, h)
  SINO
    BuscarLugarParaInsertar(M, i, h)
  FIN SI

FUNCIÓN BuscarLugarParaInsertar(MontículoFibonacci M, Nodo i, Altura h)
// Buscar el lugar donde insertar el nodo i en el montículo de Fibonacci
  Raíz = RaízDelMontículo(M)
  Si Raíz es NULL Entonces
    AgregarNodoAlRaíz(M, i, h)
  SINO
    Si Altura(h) < Altura(Raíz) Entonces
      InsertarNodoEnHijoIzquierdo(M, i, h)
    SINO
      InsertarNodoEnHijoDerecho(M, i, h)
    FIN SI
  FIN SI

FUNCIÓN AgregarNodoAlRaíz(MontículoFibonacci M, Nodo i, Altura h)
// Agregar el nodo i como raíz del montículo de Fibonacci
  Raíz = NuevoNodo(i, h)
  MontículoDelMontículo(M) = Raíz

FUNCIÓN InsertarNodoEnHijoIzquierdo(MontículoFibonacci M, Nodo i, Altura h)
// Insertar el nodo i en el hijo izquierdo del raíz
  HijoIzquierdo = HijoIzquierdoDeRaíz(M)
  Si HijoIzquierdo es NULL Entonces
    AgregarNodoAlHijoIzquierdo(M, i, h)
  SINO
    BuscarLugarParaInsertarEnHijoIzquierdo(M, i, h)
  FIN SI

FUNCIÓN InsertarNodoEnHijoDerecho(MontículoFibonacci M, Nodo i, Altura h)
// Insertar el nodo i en el hijo derecho del raíz
  HijoDerecho = HijoDerechoDeRaíz(M)
  Si HijoDerecho es NULL Entonces
    AgregarNodoAlHijoDerecho(M, i, h)
  SINO
    BuscarLugarParaInsertarEnHijoDerecho(M, i, h)
  FIN SI

FUNCIÓN AgregarNodoAlHijoIzquierdo(MontículoFibonacci M, Nodo i, Altura h)
// Agregar el nodo i como hijo izquierdo del raíz
  HijoIzquierdo = NuevoNodo(i, h)
  HijosDelMontículo(M) = HijoIzquierdo

FUNCIÓN AgregarNodoAlHijoDerecho(MontículoFibonacci M, Nodo i, Altura h)
// Agregar el nodo i como hijo derecho del raíz
  HijoDerecho = NuevoNodo(i, h)
  HijosDelMontículo(M) = HijoDerecho

FUNCIÓN BuscarLugarParaInsertarEnHijoIzquierdo(MontículoFibonacci M, Nodo i, Altura h)
// Buscar el lugar donde insertar el nodo i en el hijo izquierdo del raíz
  HijoIzquierdo = HijoIzquierdoDeRaíz(M)
  Si HijoIzquierdo es NULL Entonces
    AgregarNodoAlHijoIzquierdo(M, i, h)
  SINO
...
FUNCIÓN BuscarLugarParaInsertarEnHijoDerecho(MontículoFibonacci M, Nodo i, Altura h)
// Buscar el lugar donde insertar el nodo i en el hijo derecho del raíz
  HijoDerecho = HijoDerechoDeRaíz(M)
  Si HijoDerecho es NULL Entonces
    AgregarNodoAlHijoDerecho(M, i, h)
  SINO
...
FUNCIÓN NuevoNodo(Nodo i, Altura h)
// Crear un nuevo nodo con el valor i y la altura h
  RETORNAR Nuevo Nodo(i, h)

FUNCIÓN RaízDelMontículo(MontículoFibonacci M)
// Devolver la raíz del montículo de Fibonacci
  RETORNAR MontículoDelMontículo(M)

FUNCIÓN HijoIzquierdoDeRaíz(MontículoFibonacci M)
// Devolver el hijo izquierdo de la raíz del montículo de Fibonacci
  RETORNAR HijosDelMontículo(M)[0]

FUNCIÓN HijoDerechoDeRaíz(MontículoFibonacci M)
// Devolver el hijo derecho de la raíz del montículo de Fibonacci
  RETORNAR HijosDelMontículo(M)[1]
```

-   La función `NoEsCuadrada` verifica si la matriz de adyacencia del grafo es cuadrada, y si no lo es, rellena con 0 las filas/columnas restantes.

-   La función `AgregarNodoAlMontículo` agrega un nodo al montículo de Fibonacci con una altura determinada.

-   La función `InsertarNodoEnMontículo` inserta un nodo en el lugar correcto del montículo de Fibonacci.

-   Las funciones `BuscarLugarParaInsertar`, `AgregarNodoAlRaíz`, `InsertarNodoEnHijoIzquierdo` e `InsertarNodoEnHijoDerecho` se utilizan para encontrar el lugar donde insertar un nodo en el montículo de Fibonacci.

-   Las funciones `NuevoNodo`, `RaízDelMontículo`, `HijoIzquierdoDeRaíz` y `HijoDerechoDeRaíz` crean nuevos nodos y devuelven la raíz, hijo izquierdo o hijo derecho del montículo de Fibonacci.
...
![](imagenes/Diagrama de flujo Dijkstra Fibo.png)

##### Pseudocódigo con Complejidad Temporal

```         
FUNCIÓN DijkstraFibo(Grafo G, Nodo s, Nodo t)
  // Verificar si el grafo es conexo
  SI NoEsConexo(G) Entonces
    RETORNAR "Error: El grafo no es conexo"
  FIN SI
  ...
  // Crear un montículo de Fibonacci para almacenar las distancias
  MontículoFibonacci M
  AgregarNodoAlMontículo(M, s, 0)

  // Iterar sobre los nodos en el montículo de Fibonacci (O(V log V))
  Mientras M no esté vacío Hacer
    // Extraer el nodo con la distancia más pequeña del montículo (O(log V))
    v = ExtraerMinimoDeMontículo(M)
    d[v] = Distancia(v, s)

    // Para cada arista incidente al nodo extraído (O(E))
    Para cada arista (u, v) en E hacer
      Si u != v Entonces
        // Actualizar la distancia y predecesor del nodo destino según sea necesario (O(1))
        d[u] = Distancia(u, s) + peso(u, v)
        AgregarNodoAlMontículo(M, u, d[u])
      FIN SI
    FIN PARA

  // Verificar si la distancia desde el nodo s hasta el nodo t es infinita (O(V))
  Si d[t] == INFINITO Entonces
    RETORNAR "La distancia entre los nodos s y t es infinita"
  FIN SI

  // Construir el camino más corto desde el nodo s hasta el nodo t utilizando el predecesor de cada nodo (O(V))
  camino = []
  v = t
  Mientras v != s Hacer
    camino.insertar al principio(v)
    v = Predecesor(v, s)
  FIN MIENTRAS

  // Devolver el camino más corto y la distancia entre los nodos s y t (O(1))
  RETORNAR (camino, d[t])

FUNCIÓN NoEsConexo(Grafo G) // O(V + E)
  // Verificar si el grafo es conexo
  SI ExisteCicloEnG(G) Entonces
    RETORNAR Verdadero
  FIN SI
...
FUNCIÓN NoExisteCaminoDesdeS(HastaT, Grafo G) // O(E)
  // Verificar si existe un camino desde el nodo origen s hasta el nodo destino t
  Para cada arista (u, v) en E hacer
    Si u == s y v == t Entonces
      RETORNAR Verdadero
...
FUNCIÓN Distancia(v, s) // O(1)
  // Devolver la distancia desde el nodo origen s hasta el nodo v
  RETORNAR d[v]

FUNCIÓN peso(u, v) // O(1)
  // Devolver el peso de la arista entre los nodos u y v
  RETORNAR pesoDeLaArista(u, v)

FUNCIÓN ExisteCicloEnG(Grafo G) // O(V + E)
  // Verificar si existe un ciclo en el grafo
  Para cada nodo v en G hacer
    Si d[v] != INFINITO Entonces
      // Buscar un ciclo utilizando la función de búsqueda DFS (O(V + E))
      SI HayCiclo(v, G) Entonces
        RETORNAR Verdadero
      FIN SI
    FIN SI
...
FUNCIÓN HayCiclo(nodo v, Grafo G) // O(V + E)
  // Buscar un ciclo utilizando la función de búsqueda DFS
  visited[v] = Verdadero
  Para cada arista (u, v) en E hacer
    Si u == v Entonces
...
FUNCIÓN AgregarNodoAlMontículo(MontículoFibonacci M, Nodo v, Distancia d) // O(log V)
  // Agregar un nodo al montículo de Fibonacci
  InsertarNodoEnMontículo(M, v, d)

FUNCIÓN ExtraerMinimoDeMontículo(MontículoFibonacci M) // O(log V)
  // Extraer el nodo con la distancia más pequeña del montículo
  RETORNAR MinimoDelMontículo(M)

FUNCIÓN InsertarNodoEnMontículo(MontículoFibonacci M, Nodo v, Distancia d) // O(log V)
  // Insertar un nodo en el montículo de Fibonacci
  InsertarNodoEnPosiciónCorrecta(M, v, d)
```

###### Explicación del pseudocódigo

1.  **Verificar si el grafo es conexo**: La función `NoEsConexo(G)` verifica si el grafo es conexo utilizando la función `ExisteCicloEnG(G)`, que busca un ciclo en el grafo mediante una búsqueda DFS. Si se encuentra un ciclo, el grafo no es conexo y la función devuelve un error.

2.  **Verificar si existe un camino desde el nodo origen s hasta el nodo destino t**: La función `NoExisteCaminoDesdeS(HastaT, G)` recorre todas las aristas del grafo para verificar si existe un camino desde el nodo origen s hasta el nodo destino t. Si no existe un camino, la función devuelve un mensaje de error.

3.  **Inicializar distancias y crear montículo de Fibonacci**: La función inicializa las distancias desde el nodo origen s hasta cada nodo en el grafo con valor infinito, excepto para el nodo s que se establece en 0. Luego, crea un montículo de Fibonacci y agrega el nodo s con distancia 0 al montículo.

4.  **Iterar sobre los nodos del montículo**: La función itera sobre los nodos en el montículo de Fibonacci utilizando la función `ExtraerMinimoDeMontículo(M)` para extraer el nodo con la distancia más pequeña. Este proceso se repite hasta que el montículo esté vacío.

5.  **Actualizar distancias y predecesores**: Para cada arista incidente al nodo extraído, la función actualiza la distancia y predecesor del nodo destino según sea necesario.

6.  **Construir camino más corto**: Finalmente, la función verifica si la distancia desde el nodo s hasta el nodo t es infinita y devuelve un mensaje de error si es así. De lo contrario, construye el camino más corto desde el nodo s hasta el nodo t utilizando el predecesor de cada nodo.

7.  **Devolver camino más corto y distancia**: La función devuelve el camino más corto y la distancia entre los nodos s y t.

###### Análisis de la complejidad temporal:

-   La función `NoEsConexo(G)` tiene una complejidad temporal de $O(V + E)$ porque utiliza la función `ExisteCicloEnG(G)`, que busca un ciclo en el grafo mediante una búsqueda DFS.
-   La función `NoExisteCaminoDesdeS(HastaT, G)` tiene una complejidad temporal de $O(E)$ porque recorre todas las aristas del grafo para verificar si existe un camino desde el nodo origen s hasta el nodo destino t.
-   Las funciones `Distancia(v, s)` y `peso(u, v)` tienen ambas una complejidad temporal de $O(1)$ porque solo devuelven valores precalculados.
-   La función `ExisteCicloEnG(G)` tiene una complejidad temporal de $O(V + E)$ porque utiliza la función de búsqueda DFS para buscar un ciclo en el grafo.
-   La función `HayCiclo(nodo v, Grafo G)` tiene una complejidad temporal de $O(V + E)$ porque utiliza la función de búsqueda DFS para buscar un ciclo en el subgrafo inducido por el nodo v.
-   Las funciones `AgregarNodoAlMontículo(MontículoFibonacci M, Nodo v, Distancia d)` y `ExtraerMinimoDeMontículo(MontículoFibonacci M)` tienen ambas una complejidad temporal de $O(\log V)$ porque utilizan el montículo de Fibonacci para insertar y extraer nodos.
-   La función `InsertarNodoEnPosiciónCorrecta(M, v, d)` tiene una complejidad temporal de $O(\log V)$ porque utiliza el montículo de Fibonacci para insertar un nodo en la posición correcta.

En conclusión, la complejidad temporal del algoritmo es $O(V \log V + E)$ porque se repite el proceso de extraer el nodo con la distancia más pequeña del montículo de Fibonacci un número igual al número de nodos en el grafo, lo que tiene una complejidad temporal de $O(\log V)$, y se actualizan las distancias y predecesores de los nodos incidentes a cada arista, lo que tiene una complejidad temporal de $O(E)$.
